...
public class ApiGatewayConfiguration {
	...
	RouteLocator expenseTrackRoute(RouteLocatorBuilder builder) {
		...
		return builder.routes()
				...
				.route("user-actions",
						r -> r.path("/api/v1/users/**", "/api/v1/admin/**")
								.filters(f -> f.addRequestHeader("x-source", "gateway")).uri("http://localhost:8080"))
				...
				.route("login-Ui",
						r -> r.path("/users", "/dashboard", "/users/edit", "/editprofile", "/", "/api/**", "/css/**",
								"/js/**")
								...
								exchange.getPrincipal().cast(Authentication.class).flatMap(authentication -> {
									...
									ServerHttpRequest request = exchange.getRequest().mutate()
											.header("X-Username", username).header("X-Roles", roles)
											.header("X-Id", id)
											.build();
